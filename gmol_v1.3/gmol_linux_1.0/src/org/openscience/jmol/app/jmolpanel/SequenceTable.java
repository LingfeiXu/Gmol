...
public class SequenceTable extends JDialog {
	...
	JTextField fromText3 = null;
	JTextField endText3 = null;
	JTextField fromText4 = null;
	JTextField endText4 = null;
	...
	JComponent constructUpPanel() {
		...
		fifPanel.add(spLabel3);
		fifPanel.add(spChoice3);
		thrPanel.add(fifPanel,BorderLayout.NORTH);
		
		
		JPanel sixPanel = new JPanel();
		sixPanel.add(chrLabel3);
		sixPanel.add(chrChoice3);
		sixPanel.add(fromLabel3);
		sixPanel.add(fromText3);
		sixPanel.add(endLabel3);
		sixPanel.add(endText3);
		thrPanel.add(sixPanel,BorderLayout.SOUTH);
		...
		resourceChoice.addItem(GT._("From_UCSC_Genome_Browser"));
		...
		resourceChoice.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent ie) {
				if (ie.getStateChange() == 1) {
					...
					else if (ie.getItem().toString().equals("From_UCSC_Genome_Browser")) {
						localBaseDIR = viewer.getLocalSeqPath();
						upPanel.remove(fstPanel);
